...
而且那個雜湊算的是「前 1KB 被切成幾塊」的結構，不只是內容——同一個檔案換個 CDN
節點分塊方式不同，雜湊就不一樣，於是跳出根本不存在的更新。WhatsApp 那類
App 在 Obtainium 上一直假更新就是這個：它的連結是固定檔名 `WhatsApp.apk` 加一串
會過期的簽名參數，檔名沒有版本、網址每次還都不同，Obtainium 只能用猜的。

**別叫人把 `/latest.apk` 填進 Obtainium。** 那是個不帶版本的固定網址，填它等於
主動退回上面那條猜版本的路。要填的是這一頁的網址，版本從檔名讀。
